...
import { pluginRoot, vencordRoot, resultsDir, vencordRequire as require, uiRequire } from "../paths.mjs";
...
const bundle = await build({
    stdin: { contents: 'import * as React from "react"; import { createRoot } from "react-dom/client"; import Tab from "./components/profileSetsTab"; createRoot(document.getElementById("app")).render(<Tab />);', loader: "tsx", resolveDir: root, sourcefile: "ui-entry.tsx" },
...
#app { max-width:760px; margin:auto; } button { font:inherit; } .vc-btn-base { border:1px solid var(--border-subtle); color:var(--text-primary); border-radius:8px; padding:8px 12px; cursor:pointer; background:var(--background-surface-higher); font-size:13px; }.vc-btn-primary { background:#5865f2;color:#fff; }button:disabled{opacity:.55;cursor:default;}
.test-modal-backdrop { position:fixed; inset:0; overflow-y:auto; background:#0009; padding:20px; z-index:10; }
.test-modal { box-sizing:border-box; width:min(660px,100%); margin:auto; padding:24px; border-radius:16px; background:var(--background-primary); }
...
const capture = async name => {
    await page.screenshot({ path: join(resultsDir, name + ".png"), fullPage: true });
};
try {
    ...
    assert.equal(await page.$eval(".vc-profile-presets-list-container", el => el.children.length), 5);
    ...
    const gifPath = join(resultsDir, "upload.gif");
    ...
    await page.waitForFunction(() => window.__ui.dispatches.some(action => action.type === "USER_PROFILE_SETTINGS_SET_PENDING_CHANGES") || window.__ui.toasts.some(toast => toast.type === 1));
    ...
    console.log("UI_DIAGNOSTICS", JSON.stringify({ errors, toasts: await page.evaluate(() => window.__ui?.toasts ?? []) }));
    await writeFile(join(resultsDir, "ui.json"), JSON.stringify({ passed: results.length, results, browserErrors: errors, scope: "Actual plugin components with mocked Discord services and modal shell." }, null, 2));
    await browser.close();
